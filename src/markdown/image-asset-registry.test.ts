...
describe('image asset registry', () => {
  it('imports image blobs as stable asset refs without data URLs', async () => {
    ...
    expect(assetUrl).toMatch(/^aislenote-asset:\/\/\/assets\/asset-/)
  ...
  it('keeps stable asset refs when normalizing markdown for persistence', () => {
    const markdown = normalizeMarkdownImageSourcesForPersistence('![pixel](aislenote-asset:///assets/asset-test.png)')

    expect(markdown).toBe('![pixel](aislenote-asset:///assets/asset-test.png)')
  ...
  it('keeps markdown asset links stable when normalizing persistence sources', () => {
    const markdown = normalizeMarkdownAssetSourcesForPersistence('[report](aislenote-asset:///assets/asset-test.pdf)')

    expect(markdown).toBe('[report](aislenote-asset:///assets/asset-test.pdf)')
  })
  ...
  it('keeps escaped media asset links and media metadata stable when normalizing persistence sources', () => {
    const markdown = normalizeMarkdownAssetSourcesForPersistence(
      '[song [demo\\].mp3](aislenote-asset:///assets/song.mp3#aislenote-media=width=320)',
    )

    expect(markdown).toBe('[song [demo\\].mp3](aislenote-asset:///assets/song.mp3#aislenote-media=width=320)')
  })
})
