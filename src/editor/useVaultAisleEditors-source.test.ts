...
describe('vault aisle editor task checkbox wiring', () => {
  ...
  it('keeps lifecycle display-restoration guards while snapshots use dirty-only live reads', () => {
    ...
    expect(source).toContain('!useCachedProgrammaticMarkdown &&\n      meta.userEditedSinceProgrammaticUpdate')
    expect(source).not.toContain('meta.aisleId === activeEditorAisleIdRef.current || meta.userEditedSinceProgrammaticUpdate')
  ...
  it('does not let pending programmatic mirror refreshes hide later user edits', () => {
    ...
    expect(source).toContain('if (!meta.userEditedSinceProgrammaticUpdate) {')
    expect(source).toContain("commitEditorMarkdown(meta, editor, 'user')")
    expect(source).not.toContain("meta.programmaticMarkdownUpdatePending && !meta.userEditedSinceProgrammaticUpdate\n                  ? 'programmatic'\n                  : 'user'")
  ...
  it('refreshes tag autocomplete when editor query state can change', () => {
    ...
    expect(source.match(/onTagAutocompleteQueryChangeRef\.current\?\.\(\)/g) ?? []).toHaveLength(3)
  })
