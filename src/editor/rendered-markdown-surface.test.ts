...
describe('rendered Markdown surface contract', () => {
  it('defines one shared surface class for active editors and inactive previews', () => {
    expect(RENDERED_MARKDOWN_SURFACE_CLASS).toBe('aislenote-rendered-markdown-surface')
    expect(RENDERED_MARKDOWN_CLASS_NAMES.link).toBe('aislenote-rendered-markdown-link')
    expect(RENDERED_MARKDOWN_CLASS_NAMES.tableSourceLine).toBe('aislenote-rendered-markdown-table-source-line')
  })

  it('normalizes heading classes to supported Markdown heading levels', () => {
    expect(getRenderedMarkdownHeadingClassName(1)).toBe('aislenote-rendered-markdown-heading aislenote-rendered-markdown-heading-1')
    expect(getRenderedMarkdownHeadingClassName(99)).toBe('aislenote-rendered-markdown-heading aislenote-rendered-markdown-heading-6')
    expect(getRenderedMarkdownHeadingLineClassName(0)).toBe('aislenote-rendered-markdown-heading-line aislenote-rendered-markdown-heading-line-1')
  })
