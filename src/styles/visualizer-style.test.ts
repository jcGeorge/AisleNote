...
describe('visualizer styles', () => {
  ...
  it('keeps hierarchy chips from changing compact graph node dimensions', () => {
    ...
    const labelRule = extractRule(css, '.visualizer-node-label')
    const labelLineRule = extractRule(css, '.visualizer-node-label-line')
    ...
    expect(labelRule).toContain('max-height: 2.1em;')
    ...
    expect(labelRule).toContain('line-height: 1.05;')
    expect(labelLineRule).toContain('display: block;')
    expect(labelLineRule).toContain('text-overflow: clip;')
    expect(labelLineRule).toContain('white-space: nowrap;')
