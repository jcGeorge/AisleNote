...
function renderMediaPlayer(kind: 'audio' | 'video') {
  return renderToStaticMarkup(
    <MediaPlayer
      src={kind === 'audio' ? 'aislenote-asset:///assets/song.mp3' : 'aislenote-asset:///assets/clip.mp4'}
      kind={kind}
...
function expectSharedMediaControls(html: string, title: string) {
  expect(html.indexOf('class="aislenote-media-title"')).toBeLessThan(html.indexOf('aislenote-media-play-btn'))
  ...
  expect(html).toContain('aislenote-media-speed-btn')
  ...
  expect(html).toContain('aislenote-media-icon aislenote-media-icon-volume-full')
  expect(html).toContain('aislenote-media-volume-slider')
  ...
  expect(html).toContain('aislenote-media-icon aislenote-media-icon-play')
  expect(html).toContain('app-icon-play')
  expect(html).toContain('data-app-icon="play"')
  expect(html).toContain('aislenote-media-icon aislenote-media-icon-loop')
  expect(html).toContain('aislenote-media-back-btn editor-history-toolbar-btn editor-history-toolbar-btn-undo')
  expect(html).toContain('aislenote-media-forward-btn editor-history-toolbar-btn editor-history-toolbar-btn-redo')
  expect(html).toContain('aislenote-media-icon aislenote-media-icon-history-undo')
  expect(html).toContain('aislenote-media-icon aislenote-media-icon-history-redo')
  expect(html).toContain('M3 7v6h6')
  expect(html).toContain('M21 17a9 9 0 0 0-9-9 9 9 0 0 0-6 2.3L3 13')
  expect(html).toContain('M21 7v6h-6')
  expect(html).toContain('M3 17a9 9 0 0 1 9-9 9 9 0 0 1 6 2.3l3 2.7')
  expect(html).not.toContain('editor-history-toolbar-icon')
  expect(html).not.toContain('editor-history-toolbar-arc')
  expect(html).not.toContain('editor-history-toolbar-head')
...
describe('MediaPlayer', () => {
  ...
  it('renders video controls with the same centered title row and icon-only buttons', () => {
    ...
    expect(html).toContain('aislenote-media-viewport')
  ...
  it('initializes preview controls from playback metadata without requiring persistence props', () => {
    const html = renderToStaticMarkup(
      <MediaPlayer
        src="aislenote-asset:///assets/song.mp3#aislenote-media=speed=1.5,volume=125"
        kind="audio"
        label="Metadata Song"
      />,
    )
  ...
  it('uses medium and muted volume icons without visible percentage text', () => {
    const medium = renderToStaticMarkup(
      <MediaPlayer src="aislenote-asset:///assets/song.mp3#aislenote-media=volume=50" kind="audio" label="Medium" />,
    )
    const muted = renderToStaticMarkup(
      <MediaPlayer src="aislenote-asset:///assets/song.mp3#aislenote-media=volume=0" kind="audio" label="Muted" />,
    )

    expect(medium).toContain('aria-label="Volume 50%"')
    expect(medium).toContain('aislenote-media-icon aislenote-media-icon-volume-medium')
    expect(medium).not.toContain('>50%</button>')
    expect(muted).toContain('aria-label="Volume 0%"')
    expect(muted).toContain('aislenote-media-icon aislenote-media-icon-volume-muted')
  ...
  it('hides media file extensions from visible player titles', () => {
    const audio = renderToStaticMarkup(
      <MediaPlayer src="aislenote-asset:///assets/song.mp3" kind="audio" label="Miho Komatsu - Mystery.mp3" />,
    )
    const video = renderToStaticMarkup(
      <MediaPlayer src="aislenote-asset:///assets/clip.webm" kind="video" label="Creed - One Last Breath.MP4" />,
    )
  ...
  it('styles shared media layout and active loop state explicitly', () => {
    ...
    expect(css).toContain('.aislenote-media-controls')
    expect(css).toContain('container: aislenote-media-player / inline-size;')
    expect(css).toContain("'play loop speed volume current slider duration back forward'")
    expect(css).toContain('@container aislenote-media-player (max-width: 34rem)')
    expect(css).toContain('minmax(2.75rem, 1fr)')
    expect(css).toContain('width: 1.8rem;')
    expect(css).toContain('width: 1.95rem;')
    expect(css).toContain('@container aislenote-media-player (max-width: 24rem)')
    ...
    expect(css).toContain('@container aislenote-media-player (max-width: 20rem)')
    expect(css).toContain("'current slider slider slider slider duration'")
    expect(css).toContain("'play loop speed volume back forward'")
    expect(css).toContain('@container aislenote-media-player (max-width: 12rem)')
    ...
    expect(css).toContain('.aislenote-media-volume-popup')
    expect(css).toContain("bottom: calc(100% + 0.3rem);")
    expect(css).toContain('z-index: 3300;')
    expect(css).toContain('transform: translateX(-50%);')
    expect(css).toContain(".toastui-editor .ProseMirror .aislenote-media-player[data-media-kind='audio']")
    expect(css).toContain(".aisle-edit-preview .aislenote-media-player[data-media-kind='audio'] {\n  overflow: visible;")
    expect(css).toContain(".aislenote-media-player[data-media-kind='audio'] .aislenote-media-speed-menu")
    expect(css).toContain(".aislenote-media-player[data-media-kind='audio'] .aislenote-media-volume-popup")
    expect(css).toContain('top: auto;')
    expect(css).toContain('left: 0;')
    expect(css).toContain(
      '.toastui-editor .ProseMirror .aislenote-media-player .aislenote-media-speed-menu',
    )
    expect(css).toContain(
    ...
    expect(css).toContain('.aislenote-media-player:focus')
    expect(css).toContain('.aislenote-media-player .aislenote-media-video:focus-visible')
    expect(css).toContain('.aislenote-media-player .aislenote-media-slider:focus')
    expect(css).toContain('.aislenote-media-player .aislenote-media-volume-slider:focus-visible')
    expect(css).toContain('outline: none !important;')
    expect(css).toContain('box-shadow: none !important;')
    expect(css).toContain('.aislenote-media-player .aislenote-media-slider::-moz-focus-outer')
    expect(css).not.toContain('.aislenote-media-btn:hover,\n.aislenote-media-btn:focus-visible')
    expect(css).not.toContain('.aislenote-media-speed-option:hover,\n.aislenote-media-speed-option:focus-visible')
    expect(css).toContain(".aislenote-media-loop-btn[aria-pressed='true']")
    ...
    expect(css).not.toContain(".aislenote-media-player[data-media-kind='audio'] .aislenote-media-controls")
