...
export function registerFileIpc({ ipcMain, dialog, storageSession = null }) {
  ...
  ipcMain.handle('open-notebook-folder-import', async () => {
    if (!dialog || typeof dialog.showOpenDialog !== 'function') {
      return { canceled: false, ok: false, error: 'Notebook folder import is unavailable.' }
    }

    const openResult = await dialog.showOpenDialog({
      title: 'Import notebook folder',
      properties: ['openDirectory'],
    })

    if (openResult.canceled || !openResult.filePaths?.[0]) return { canceled: true }

    const folderPath = path.resolve(openResult.filePaths[0])
    const notesRootPath = getHybridStorageRoot(folderPath)
    const manifestPath = path.join(notesRootPath, 'manifest.json')
    try {
      const folderStats = lstatSync(folderPath)
  ...
  ipcMain.handle('open-markdown-folder-import', async () => {
    if (!dialog || typeof dialog.showOpenDialog !== 'function') {
      return { canceled: false, ok: false, error: 'Markdown folder import is unavailable.' }
    }
